(ns forgeworker.actor
  "ForgeWorkerActor — the ISCO-08 7221 forge-shop scheduling/logistics
  coordination actor as a `langgraph.graph/state-graph` (ADR-2607121000
  / CLAUDE.md Actors section). One graph run = one forge-shop-
  ...
  cloud-itonami-isco-7111's housebuilder.actor (the closest domain
  shape in this wave — same physical-safety-domain scaffold).
  ...
  The unconditional invariant: the Forge Worker Advisor can never
  directly commit an operating record the ForgeWorkerGovernor refuses,
  and can never make this actor finalize a forging-execution decision
  (e.g. a specific hammer-strike or press-stroke) or override a
  forge-shop safety officer's judgment — every commit-record! call is
  gated behind `:decide`, and that decision class is a permanent hard
  block regardless of disposition path. This actor coordinates
  FORGE-SHOP SCHEDULING/LOGISTICS ONLY — it never performs forging work
  or authorizes press operations itself."
...
(defn build-graph
  "Build a compiled ForgeWorkerActor graph. `store` implements
  `forgeworker.store/Store`. `advisor` implements
  `forgeworker.advisor/Advisor` (defaults to `mock-advisor`).
